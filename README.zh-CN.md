...
`$` 发现按配置中的 root 顺序扫描，同一规范化 Skill 名保留第一条。希望优先使用已分发版本时，把运行时目录放在源码目录前面。扫描会跟随目录和 `SKILL.md` 软链接，但保留发现时的路径作为相对引用基准；断链和指向祖先的循环链接会跳过，深度、忽略目录和文件大小限制仍然有效。这不会重写 Skill 正文中的链接或 Bundle 显式成员路径。
